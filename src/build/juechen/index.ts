...
import { JUECHEN_QUELLE_STAND, JUECHEN_STRASSEN_URL, JUECHEN_WAHLKREISE } from "./config.js";
...
const OUTPUT_PATH = "public/data/juechen.json";

interface JuechenZeile {
  strasse: string;
  hausnummernBereich: string;
  bezirkNr: string;
}

function parseZeile(spalten: string[]): JuechenZeile | null {
...
async function ladeZeilen(): Promise<JuechenZeile[]> {
  const response = await fetch(JUECHEN_STRASSEN_URL);
  if (!response.ok) {
    throw new BuildError(`Download fehlgeschlagen (${response.status}): ${JUECHEN_STRASSEN_URL}`);
  }
  const text = await response.text();
  const zeilen = text.split(/\r?\n/).filter((z) => z.length > 0);
  const [, ...datenzeilen] = zeilen; // erste Zeile ist der Spaltenkopf

  const ergebnis: JuechenZeile[] = [];
...
async function main(): Promise<void> {
  console.log("Lade Jüchener Straßenverzeichnis ...");
  ...
  await ergaenzeBuchstabenAusnahmen("Jüchen", buchstabenAusZeilen(byStrasse), strassen);

  checkWahlkreisCount(JUECHEN_WAHLKREISE, 2);
