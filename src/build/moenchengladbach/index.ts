...
const OUTPUT_PATH = "public/data/moenchengladbach.json";

interface MoenchengladbachZeile {
  strasse: string;
  hausnummernBereich: string;
  bezirkNr: string;
}

function parseZeile(spalten: string[]): MoenchengladbachZeile | null {
...
async function ladeZeilen(): Promise<MoenchengladbachZeile[]> {
  const response = await fetch(MOENCHENGLADBACH_STRASSEN_URL);
  if (!response.ok) {
    throw new BuildError(
      `Download fehlgeschlagen (${response.status}): ${MOENCHENGLADBACH_STRASSEN_URL}`,
    );
  }
  ...
  const ergebnis: MoenchengladbachZeile[] = [];
...
async function main(): Promise<void> {
  console.log("Lade Mönchengladbacher Straßenverzeichnis ...");
  ...
  const data: StrassenDaten = {
    meta: {
      ...
      stand: MOENCHENGLADBACH_QUELLE_STAND,
      gebaut_am: new Date().toISOString().slice(0, 10),
      quellen: [MOENCHENGLADBACH_STRASSEN_URL],
      // Straße -> Wahlbezirk-Präfix aus dem Kommunalwahl-Verzeichnis,
      // Wahlbezirk-Präfix -> Landtagswahlkreis über die Klartext-
      // Einrichtungsnamen (siehe wahlkreis.ts) - kein direkter Bezug zu den
      // Stadtteil-Nummern der Landeswahlgesetz-Anlage, daher weiterhin
      // "gesetzesanlage" (der letzte Schritt zum Wahlkreis kommt aus ihr).
      zuordnung: "gesetzesanlage",
    },
    wahlkreise: MOENCHENGLADBACH_WAHLKREISE,
    strassen,
  };
