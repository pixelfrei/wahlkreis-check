...
import { HENNEF_QUELLE_STAND, HENNEF_STRASSEN_URL, HENNEF_WAHLKREISE } from "./config.js";
...
const OUTPUT_PATH = "public/data/hennef.json";

interface HennefZeile {
  strasse: string;
  hausnummernBereich: string;
  bezirkNr: number;
}

function parseZeile(spalten: string[]): HennefZeile | null {
...
async function ladeZeilen(): Promise<HennefZeile[]> {
  const response = await fetch(HENNEF_STRASSEN_URL);
  if (!response.ok) {
    throw new BuildError(`Download fehlgeschlagen (${response.status}): ${HENNEF_STRASSEN_URL}`);
  }
  const text = await response.text();
  const zeilen = text.split(/\r?\n/).filter((z) => z.length > 0);
  const [, ...datenzeilen] = zeilen; // erste Zeile ist der Spaltenkopf

  const ergebnis: HennefZeile[] = [];
...
async function main(): Promise<void> {
  console.log("Lade Hennefer Straßenverzeichnis ...");
  ...
  await ergaenzeBuchstabenAusnahmen("Hennef (Sieg)", buchstabenAusZeilen(byStrasse), strassen);

  checkWahlkreisCount(HENNEF_WAHLKREISE, 2);
  ...
  const data: StrassenDaten = {
    meta: {
      kommune: "Hennef (Sieg)",
      stand: HENNEF_QUELLE_STAND,
      gebaut_am: new Date().toISOString().slice(0, 10),
      quellen: [HENNEF_STRASSEN_URL],
      // Straße -> Stimmbezirk aus dem Straßenverzeichnis, Stimmbezirk ->
      // Landtagswahlkreis direkt aus der Landeswahlgesetz-Anlage (siehe
      // wahlkreis.ts, inkl. Korrektur des Stimmbezirks 200 statt 202).
      zuordnung: "gesetzesanlage",
    },
    wahlkreise: HENNEF_WAHLKREISE,
    strassen,
  };
