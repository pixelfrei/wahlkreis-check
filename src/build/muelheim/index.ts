...
import { MUELHEIM_QUELLE_STAND, MUELHEIM_STRASSEN_URL, MUELHEIM_WAHLKREISE } from "./config.js";
...
const OUTPUT_PATH = "public/data/muelheim.json";

interface MuelheimZeile {
  strasse: string;
  hausnummernBereich: string;
  bezirkNr: string;
}

function parseZeile(spalten: string[]): MuelheimZeile | null {
...
async function ladeZeilen(): Promise<MuelheimZeile[]> {
  const response = await fetch(MUELHEIM_STRASSEN_URL);
  if (!response.ok) {
    throw new BuildError(`Download fehlgeschlagen (${response.status}): ${MUELHEIM_STRASSEN_URL}`);
  }
  const text = await response.text();
  const zeilen = text.split(/\r?\n/).filter((z) => z.length > 0);
  const [, ...datenzeilen] = zeilen; // erste Zeile ist der Spaltenkopf

  const ergebnis: MuelheimZeile[] = [];
...
async function main(): Promise<void> {
  console.log("Lade Mülheimer Straßenverzeichnis ...");
  ...
  await ergaenzeBuchstabenAusnahmen("Mülheim an der Ruhr", buchstabenAusZeilen(byStrasse), strassen);

  checkWahlkreisCount(MUELHEIM_WAHLKREISE, 2);
  ...
  const data: StrassenDaten = {
    meta: {
      ...
      stand: MUELHEIM_QUELLE_STAND,
      gebaut_am: new Date().toISOString().slice(0, 10),
      quellen: [MUELHEIM_STRASSEN_URL],
      // Straße -> Stimmbezirk (dessen erste zwei Ziffern den Kommunalwahlbezirk
      // ergeben) aus dem Kommunalwahl-Verzeichnis, Kommunalwahlbezirk ->
      // Landtagswahlkreis aus der Landeswahlgesetz-Anlage (siehe wahlkreis.ts).
      zuordnung: "gesetzesanlage",
    },
    wahlkreise: MUELHEIM_WAHLKREISE,
    strassen,
  };
