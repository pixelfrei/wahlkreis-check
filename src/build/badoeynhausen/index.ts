...
import {
  BAD_OEYNHAUSEN_QUELLE_STAND,
  BAD_OEYNHAUSEN_STRASSEN_URL,
  BAD_OEYNHAUSEN_WAHLKREISE,
} from "./config.js";
...
const OUTPUT_PATH = "public/data/badoeynhausen.json";

interface BadOeynhausenZeile {
  strasse: string;
  hausnummernBereich: string;
  bezirkNr: string;
}

function parseZeile(spalten: string[]): BadOeynhausenZeile | null {
...
async function ladeZeilen(): Promise<BadOeynhausenZeile[]> {
  const response = await fetch(BAD_OEYNHAUSEN_STRASSEN_URL);
  if (!response.ok) {
    throw new BuildError(
      `Download fehlgeschlagen (${response.status}): ${BAD_OEYNHAUSEN_STRASSEN_URL}`,
    );
  }
  ...
  const ergebnis: BadOeynhausenZeile[] = [];
...
async function main(): Promise<void> {
  console.log("Lade Bad Oeynhausener Straßenverzeichnis ...");
  ...
  await ergaenzeBuchstabenAusnahmen("Bad Oeynhausen", buchstabenAusZeilen(byStrasse), strassen);

  checkWahlkreisCount(BAD_OEYNHAUSEN_WAHLKREISE, 2);
  ...
  const data: StrassenDaten = {
    meta: {
      ...
      stand: BAD_OEYNHAUSEN_QUELLE_STAND,
      gebaut_am: new Date().toISOString().slice(0, 10),
      quellen: [BAD_OEYNHAUSEN_STRASSEN_URL],
      // Straße -> Wahlbezirk aus dem Kommunalwahl-Verzeichnis, Wahlbezirk ->
      // Stadtteil aus opendata-wahllokale.csv (Bezirk-Name), Stadtteil ->
      // Landtagswahlkreis aus der Landeswahlgesetz-Anlage (siehe wahlkreis.ts).
      zuordnung: "gesetzesanlage",
    },
    wahlkreise: BAD_OEYNHAUSEN_WAHLKREISE,
    strassen,
  };
