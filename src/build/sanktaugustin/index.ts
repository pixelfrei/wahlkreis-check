...
import {
  SANKT_AUGUSTIN_BBOX,
  SANKT_AUGUSTIN_GEBREF_MARKER,
  SANKT_AUGUSTIN_GEMEINDE,
...
const OUTPUT_PATH = "public/data/sanktaugustin.json";

async function main(): Promise<void> {
  console.log("Lade Sankt Augustiner Gemarkungsgrenzen (ALKIS) ...");
  const polygone = await ladeGemarkungen(SANKT_AUGUSTIN_BBOX, SANKT_AUGUSTIN_GEMEINDE);
  console.log(`  ${polygone.length} Gemarkungen geladen: ${polygone.map((p) => p.name).join(", ")}`);

  console.log("Lade und filtere landesweite Gebäudereferenzen auf Sankt Augustin ...");
  const punkte = await ladeGebrefAdressen(SANKT_AUGUSTIN_GEBREF_MARKER);
  ...
  const data: StrassenDaten = {
    meta: {
      ...
      stand: SANKT_AUGUSTIN_QUELLE_STAND,
      gebaut_am: new Date().toISOString().slice(0, 10),
      quellen: [ALKIS_KATASTERBEZIRK_URL, GEBREF_URL],
      // Adresse -> Gemarkung (= Stadtteil, bis auf "Ort" - siehe
      // wahlkreis.ts) per Punkt-in-Polygon-Abgleich, Stadtteil -> Wahlkreis
      // aus der Landeswahlgesetz-Anlage.
      zuordnung: "berechnet",
    },
    wahlkreise: SANKT_AUGUSTIN_WAHLKREISE,
    strassen,
  };
